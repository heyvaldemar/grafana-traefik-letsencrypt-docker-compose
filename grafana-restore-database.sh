#!/usr/bin/env bash
# grafana-restore-database.sh [backup-file-name]
#
# Replaces the Grafana database with one of the dumps the backups service wrote.
#
#   ./grafana-restore-database.sh               list and ask
#   ./grafana-restore-database.sh <file-name>   restore that one
#
# EVERY PATH, NAME AND CREDENTIAL COMES FROM THE RUNNING BACKUPS CONTAINER.
# The previous version carried grafanadb and grafanadbuser as literals and its
# own copy of the backup directory, so any of the three set differently in
...
DB_NAME="$(env_of GRAFANA_DB_NAME)"; DB_USER="$(env_of GRAFANA_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using Grafana." >&2
  exit 1
fi
