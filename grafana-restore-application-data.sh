...
BACKUP_PATH="/srv/grafana-application-data/backups/"
RESTORE_PATH="/opt/bitnami/grafana/data/"
BACKUP_PREFIX="grafana-application-data"

echo "--> All available application data backups:"

for entry in $(docker container exec -it "$GRAFANA_BACKUPS_CONTAINER" sh -c "ls $BACKUP_PATH")
do
  echo "$entry"
done
...
docker exec -it "$GRAFANA_BACKUPS_CONTAINER" sh -c "rm -rf ${RESTORE_PATH}* && tar -zxpf ${BACKUP_PATH}${SELECTED_APPLICATION_BACKUP} -C /"
