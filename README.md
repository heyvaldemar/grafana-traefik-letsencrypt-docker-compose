...
[![OpenSSF Best Practices](https://www.bestpractices.dev/projects/14879/badge)](https://www.bestpractices.dev/projects/14879)
...
Every release from v1.7.4 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `grafana-traefik-letsencrypt-docker-compose-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `grafana-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `grafana-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob grafana-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --bundle grafana-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/grafana-traefik-letsencrypt-docker-compose/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact grafana-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --provenance-path grafana-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/grafana-traefik-letsencrypt-docker-compose
```
