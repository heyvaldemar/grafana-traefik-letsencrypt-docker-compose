#!/bin/bash
# End-to-end tests for the grafana-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-grafana}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-grafana-traefik-letsencrypt-docker-compose.yml}"
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./grafana-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./grafana-restore-database.sh "$(basename "$baseline")" \
...
test_data_restore_roundtrip() {
  ...
  echo "  restoring $(basename "$newest") with ./grafana-restore-application-data.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./grafana-restore-application-data.sh "$(basename "$newest")" \
    || { fail "the shipped restore script failed"; return 1; }
